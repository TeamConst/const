...
const SignupForm = () => {
  ...
  const validId = async () => {
    ...
    if (a.data == "아이디가있습니다") {
      아이디유효성변경("중복된 아이디가 존재합니다. 아이디를 변경해주세요.");
      아이디유효성검사변경(false);
    } else {
              ...
              >
                ...
                <Grid container spacing={5}>
                  ...
                  <Grid item xs={12}>
                    ...
                    <Typography>
                      (주의) 본인의 메타마스크 계정이 맞는지 다시 한번 확인
                      해주세요 <br></br> 현재 메타마스크 잔액 :{" "}
                      {`${Math.floor(accountBalance)}/eth`}
                    </Typography>
                    <TextField
                      fullWidth
                      id="artistAddress"
